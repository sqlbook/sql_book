...
export PGDATABASE="${POSTGRES_MAINTENANCE_DB:-postgres}"

psql -c "CREATE ROLE sqlbook_readonly WITH LOGIN PASSWORD '$POSTGRES_READONLY_PASSWORD';" || true
psql -d sqlbook_events_$1 -c "GRANT USAGE ON SCHEMA public TO sqlbook_readonly;"
psql -d sqlbook_events_$1 -c "REVOKE SELECT ON ALL TABLES IN SCHEMA public FROM sqlbook_readonly;"
psql -d sqlbook_events_$1 -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public REVOKE SELECT ON TABLES FROM sqlbook_readonly;"
